describe('Ghost under ripper monkeys', function () {
  it('visits ghost home and survives monkeys', function () {
    cy.visit('http://localhost:2368/')
    cy.wait(1000)
    randomEvent(100)
  })

  it('visits ghost admin and survives monkeys', function () {
    cy.visit('http://localhost:2368/ghost/')
    cy.title().should('contain', 'Sign In')
    cy.get('input[name="identification"]').type('test@diegorbaquero.com')
    cy.get('input[name="password"]').type('hola123456')
...
function randomLink () {
  ...
  cy.get('a').then($links => {
    ...
    if (randomLink.href.indexOf('http://localhost') === 0)
      cy.wrap(randomLink).click({force: true})
  })
}

function randomButton () {
  if (Cypress.$('button').length <= 0) return;
...
function randomCombo () {
  if (Cypress.$('select').length <= 0) return;
...
function randomInput () {
  if (Cypress.$('input[type="text"]').length <= 0) return;
